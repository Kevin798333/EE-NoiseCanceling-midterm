...
while True:
    ...
    if choice == "1":
        ...
        if filtered and mixed:
            ...
            wavfile.write("mixed_preview.wav", rate_m, mix_data)
            ...
            plt.subplot(2, 1, 2)
            plt.title("Filtered (Noise Cancelled) Audio")
            ...
            plt.show()
            while True:
                select = input("choose between the before(1) and after(2) to hear the audio type (0) to exit: ")
                if select == "1":
                    print("Will Playing original (noisy) mix...")
                    winsound.PlaySound("mixed_preview.wav", winsound.SND_FILENAME)
                elif select == "2":
                    print("Will now play the output audio...")
